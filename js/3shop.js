/* Levor Energy - Shop (REAL Cart + UGX/USD + Quote/Invoice via Backend) */
...
/* Totals (computed in UGX + displayed in selected currency) */
function calcTotals() {
...
/* ✅ REAL issuing: calls backend function to generate doc number, PDF, email, store */
async function issueDocument(type) {
  ...
  const payload = {
    ...
    cart,                // { serviceId: qty }
    client,
    totalsUGX: { subtotalUGX, vatUGX, grandUGX } // optional; server should verify
  };
  ...
  try {
    ...
    if (data?.pdfUrl) {
      ...
      alert("Document created but no PDF URL returned. Check backend response.");
    }
  } catch (err) {
    console.error(err);
    alert(
      "Unable to issue document. Make sure the Netlify Function is deployed:\n" +
      "/.netlify/functions/createDocument\n\n" +
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ currency change refreshes prices/totals + rate label
  ...
  // ✅ now uses backend (real issuing)
  document.getElementById("downloadQuoteBtn")?.addEventListener("click", () => issueDocument("quote"));
  document.getElementById("downloadInvoiceBtn")?.addEventListener("click", () => issueDocument("invoice"));
});
